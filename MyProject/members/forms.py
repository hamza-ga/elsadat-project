...
class EditPatientForm(UserChangeForm):
    ...
    email = forms.EmailField(label = 'البريد الاكتروني' ,widget=forms.EmailInput(attrs= {'class': 'form-control'}))
    first_name = forms.CharField(label = 'الاسم الاول',max_length=50, widget=forms.TextInput(attrs= {'class': 'form-control'}))
    last_name = forms.CharField(label = 'الاسم الثاني',max_length=50, widget=forms.TextInput(attrs= {'class': 'form-control'}))
    birthday = forms.DateField(label = 'تاريخ الميلاد',widget=forms.DateInput(attrs= {'class': 'form-control'}))
    phone_number = forms.CharField(label = 'رقم التليفون',max_length=100, widget=forms.TextInput(attrs= {'class': 'form-control'}))
    gender = forms.ChoiceField(choices=GENDER_CHOICES ,label = 'النوع', widget=forms.Select(attrs= {'class': 'gender field'}))
    blood = forms.ChoiceField(choices=BLOOD_TYPES, label = 'فصيلة الدم', widget=forms.Select(attrs= {'class': 'kind field'}))
    city = forms.CharField(label = 'الدوله', widget=forms.TextInput(attrs= {'class': 'field'}))
